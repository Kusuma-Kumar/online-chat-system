...
void *handle_client(void *arg) {
    ...
    char buf[BUF_SIZE];
    char nickname[50] = "User unknown";
    int bytes_recieved; 

    while((bytes_recieved = recv(conn_fd, buf, BUF_SIZE, 0)) > 0) {
        ...
        if(strncmp(buf, "/nick ", 6) == 0) {
            ...
            strncpy(nickname, buf + 6, sizeof(nickname) - 1);
            nickname[sizeof(nickname) - 1] = '\0';

            printf("User %s:%d changed nickname from %s to %s\n", client_ip, client_port, old_nick, nickname);
            continue;
        }

        printf("Received message from %s (%s:%d)\n", nickname, client_ip, client_port);
        // will echo back the message to the server?
        send(conn_fd, buf, bytes_recieved, 0);
    }

    printf("User %s (%s:%d) disconnected.\n", nickname, client_ip, client_port);
    close(conn_fd);
    return NULL;
}
